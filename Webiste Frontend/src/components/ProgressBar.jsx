...
export function ProgressBar({ value, maxValue = 100, segments = 10 }) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {/* Progress bar */}
      <div
        className="relative w-[244px] h-9 rounded-full overflow-hidden shadow-[0_0_15px_rgba(0,0,0,0.3)] border border-white/5"
        style={{
      ...
      >
        <div className="absolute inset-0 flex items-center px-1 pl-2 gap-[3px]">
          {Array.from({ length: segments }).map((_, i) => (
            <div
              key={i}
              className={`h-5 flex-1 transition-all duration-300 ease-in-out ${
                i === 0 ? 'rounded-l-full' : ''
              } ${
